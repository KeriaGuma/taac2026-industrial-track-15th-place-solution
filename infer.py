...
_FALLBACK_MODEL_CFG = {
    ...
    'seq_encoder_type_by_domain': 'seq_a:transformer',
    'seq_query_counts': 'seq_a:4,seq_b:1,seq_c:1,seq_d:1',
    'hidden_mult': 8,
    'dropout_rate': 0.01,
    'seq_top_k': 50,
    'seq_causal': False,
    ...
    'user_dense_tokens': 4,
    'item_dense_tokens': 1,
    # V14: SE-Net per-NS-token gating. Both must be recovered or the checkpoint's
    # se_gate.* params fail strict load (use_se_net) / mismatch shape (se_reduction).
    'use_se_net': True,
    'se_reduction': 2,
    # V14_PerTokenFFN: must be recovered or the per-token W1/b1/W2/b2 params in the
    # checkpoint fail strict load against a model built with the shared fc1/fc2.
    # Fallback True matches this version (run.sh trains with --per_token_ffn);
    # the real value is recovered from train_config.json (= vars(args)).
    'per_token_ffn': True,
    # Training-only token dropout; inactive in eval/predict but kept here so
    # train_config recovery can reconstruct the checkpoint's model cfg exactly.
    'ns_spatial_dropout_p': 0.10,
    'use_inter_event_gap_session': False,
    'use_mono_time_decay_attn_bias': False,
    'mono_time_decay_domains': 'seq_a,seq_b',
    'mono_time_decay_lambda_min': 0.02,
    'mono_time_decay_lambda_init': 0.02,
    'recent_core_k': 64,
    'recent_core_depth': 3,
    'recent_core_tail': True,
}

_FALLBACK_SEQ_MAX_LENS = 'seq_a:256,seq_b:256,seq_c:512,seq_d:256'
_FALLBACK_BATCH_SIZE = 1024
...
def _feature_kwargs_from_train_config(train_config: Dict[str, Any]) -> Dict[str, Any]:
    """Recover V2 synthetic feature switches used during training."""
    return {
        'use_abs_time_user_int': bool(train_config.get('use_abs_time_user_int', True)),
        'use_sequence_summary_dense': bool(train_config.get('use_sequence_summary_dense', True)),
        'use_seq_trunc_dense': bool(train_config.get('use_seq_trunc_dense', True)),
        'use_missing_indicator_dense': bool(train_config.get('use_missing_indicator_dense', True)),
        'use_target_match_dense': bool(train_config.get('use_target_match_dense', True)),
        'use_reserved_categorical_ids': bool(
            train_config.get('use_reserved_categorical_ids', False)),
        'use_inter_event_gap_session': bool(train_config.get('use_inter_event_gap_session', False)),
        'target_match_seq_fids': train_config.get('target_match_seq_fids', ''),
        'dense_robust_user_fids': train_config.get('dense_robust_user_fids', ''),
        'dense_robust_clip': float(train_config.get('dense_robust_clip', 16.0)),
        'dense_robust_scale': bool(train_config.get('dense_robust_scale', True)),
    }
...
def main() -> None:
    ...
    if not model_dir:
        raise ValueError("MODEL_OUTPUT_PATH is required")
    if not data_dir:
        raise ValueError("EVAL_DATA_PATH is required")
    if not result_dir:
        raise ValueError("EVAL_RESULT_PATH is required")
    ...
    feature_kwargs = _feature_kwargs_from_train_config(train_config)
    logging.info(f"Recovered V2 feature kwargs: {feature_kwargs}")
    ...
    use_pin_memory = torch.cuda.is_available() and _env_flag('PIN_MEMORY', False)
    logging.info(
        "Inference DataLoader pin_memory=%s (PIN_MEMORY=%r)",
        use_pin_memory,
        os.environ.get('PIN_MEMORY'),
    )
    ...
    use_amp = bool(train_config.get('use_amp', True)) and torch.cuda.is_available()
    amp_dtype_name = str(train_config.get('amp_dtype', 'bf16')).lower()
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_loader):
            ...
            probs = torch.sigmoid(safe_logits).cpu().numpy()
