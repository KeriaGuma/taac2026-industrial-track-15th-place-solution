...
def effective_rank(X: torch.Tensor) -> float:
    """Entropy-based effective rank: exp(H(p)), p = s / sum(s).

    Args:
        X: 2D tensor (N, D). Returns NaN for shapes that cannot be SVD'd.
    """
...
# ═══════════════════════════════════════════════════════════════════════════════
# V14: ModelEMA — exponential moving average of dense (non-sparse) params.
# Ported from github/7/best/utils.py:236-286.
# ═══════════════════════════════════════════════════════════════════════════════
...
class ModelEMA:
    ...
    Maintains a shadow copy of selected trainable parameters, updated each
    training step as:  shadow = decay * shadow + (1 - decay) * param

    At evaluation/checkpoint time, ``apply_shadow`` swaps in the smoothed
    weights; ``restore`` puts the training weights back.

    Sparse Embedding params (Adagrad) should be EXCLUDED via
    exclude_param_ptrs — they have their own optimizer dynamics and EMA
    would dampen the sparse-update signal.
    """
    ...
    def __init__(
        ...
        model: nn.Module,
        decay: float = 0.999,
        exclude_param_ptrs: Optional[set] = None,
    ) -> None:
        self.decay = decay
        self.shadow: Dict[str, torch.Tensor] = {}
        self.backup: Dict[str, torch.Tensor] = {}
        exclude = exclude_param_ptrs or set()
        for name, param in model.named_parameters():
            if param.requires_grad and param.data_ptr() not in exclude:
    ...
    def resync(self, model: nn.Module, param_ptrs: set) -> None:
        """Re-sync shadow for reinitialized parameters (e.g. after
        high-cardinality embedding reinit at end of an epoch)."""
        for name, param in model.named_parameters():
